...
Begin VB.Form frmAllSY 
   ...
   Begin VB.Menu mnuAction 
      Caption         =   "&Action"
      Visible         =   0   'False
      Begin VB.Menu mnuMakeDefaultSY 
         Caption         =   "&Make as Default S.Y."
      End
...
Private Sub LoadEntries()
    ...
    While vRS.EOF = False
        ...
        With listEntries
            il = .AddItem(ReadField(vRS.Fields("SYID")))
            .ItemImage(il) = 1
            .CellText(il, 1) = ReadField(vRS.Fields("SYTitle"))
        End With
...
Public Function Form_Add()
    ...
    If frmSYEntry.ShowAdd = True Then
        Form_Refresh
    End If
...
Public Function Form_Edit()
    ...
    Dim lSYID As Integer
    
    If listEntries.RowCount < 1 Then
        Exit Function
    End If
    
    lSYID = GetTxtVal(listEntries.CellText(listEntries.Row, 0))
    
    If frmSYEntry.ShowEdit(lSYID) = True Then
        Form_Refresh
    End If
...
Public Function Form_Delete()
    ...
    If MsgBox("Are you sure you want to delete this SY entry named   '" & listEntries.CellText(listEntries.Row, 1) & "' ?", vbQuestion + vbYesNo + vbDefaultButton2) <> vbYes Then
        Exit Function
    End If
    ...
    If DeleteSY(lSYID) = True Then
        Form_Refresh
    Else
        WriteErrorLog Me.Name, "Form_Delete", "Failed on: 'DeleteSY(lSYID) = True'"
    End If
...
Private Sub mnuMakeDefaultSY_Click()
    ...
    iSYID = CInt(listEntries.CellText(listEntries.Row, 0))
    
    If modGV.SetCureentSY(iSYID) = True Then
        MsgBox "The default S.Y. is now " & iSYID & "-" & (iSYID + 1) & vbNewLine & vbNewLine & "Please restart b8 GS.", vbInformation
    End If
RAE:
End Sub
