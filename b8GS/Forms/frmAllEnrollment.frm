...
Begin VB.Form frmAllEnrollment 
   BackColor       =   &H00F5F5F5&
   BorderStyle     =   0  'None
   Caption         =   "Enrollments"
   ClientHeight    =   5685
   ...
   EndProperty
   Icon            =   "frmAllEnrollment.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
      ...
      Begin MSComctlLib.ImageList ilList 
         ...
         BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
            ...
            BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
               Picture         =   "frmAllEnrollment.frx":000C
               Key             =   ""
            EndProperty
         EndProperty
      End
   ...
   End
End
Attribute VB_Name = "frmAllEnrollment"
...
Private Sub Form_Load()
    ...
    With listEntries
        ...
        .AddColumn "Enrollemnt ID", 100   '0
        .AddColumn "S.Y. ID", 0  '1
        .AddColumn "S.Y.", 100  '2
        .AddColumn "Sem", 50  '3
        .AddColumn "Course ID", 0  '4
        .AddColumn "Course", 80  '5
        .AddColumn "Student ID", 100  '6
        .AddColumn "Last Name", 120  '7
        .AddColumn "First Name", 100  '8
        .AddColumn "Middle Name", 100  '9
        .AddColumn "Y.L.", 50  '10
        .AddColumn "Created", 100  '11
        .AddColumn "Modified", 100  '12
        .AddColumn "Remarks", 120  '13
...
Private Sub LoadEntries()
    ...
    'set SQL Expression
    sSQL = "SELECT tblEnrollment.EnrollmentID, tblEnrollment.FK_SYID, tblSY.SYTitle, tblEnrollment.Sem, tblEnrollment.FK_CourseID, tblCourse.Title, tblEnrollment.FK_StudentID, tblStudent.LastName, tblStudent.FirstName, tblStudent.MiddleName, tblStudent.YL, tblEnrollment.CreationDate, tblEnrollment.ModifiedDate, tblEnrollment.Remarks" & _
            " FROM tblCourse INNER JOIN (tblSY INNER JOIN (tblStudent INNER JOIN tblEnrollment ON tblStudent.StudentID = tblEnrollment.FK_StudentID) ON tblSY.SYID = tblEnrollment.FK_SYID) ON tblCourse.CourseID = tblEnrollment.FK_CourseID" & _
            " ORDER BY tblEnrollment.FK_SYID, tblEnrollment.Sem, tblEnrollment.FK_CourseID"
    ...
    While vRS.EOF = False
        ...
        With listEntries
            il = .AddItem(ReadField(vRS.Fields("EnrollmentID")))
            .ItemImage(il) = 1
            .CellText(il, 1) = ReadField(vRS.Fields("FK_SYID"))
            .CellText(il, 2) = ReadField(vRS.Fields("SYTitle"))
            .CellText(il, 3) = ReadField(vRS.Fields("Sem"))
            .CellText(il, 4) = ReadField(vRS.Fields("FK_CourseID"))
            .CellText(il, 5) = ReadField(vRS.Fields("Title"))
            .CellText(il, 6) = ReadField(vRS.Fields("FK_StudentID"))
            .CellText(il, 7) = ReadField(vRS.Fields("LastName"))
            .CellText(il, 8) = ReadField(vRS.Fields("FirstName"))
            .CellText(il, 9) = ReadField(vRS.Fields("MiddleName"))
            .CellText(il, 10) = modYL.YLNumToStr(CInt(ReadField(vRS.Fields("YL"))))
            .CellText(il, 11) = ReadField(vRS.Fields("Remarks"))
            .CellText(il, 12) = ReadField(vRS.Fields("CreationDate"))
            .CellText(il, 13) = ReadField(vRS.Fields("ModifiedDate"))
        End With
...
Public Function Form_Add()
    ...
    If frmEnrollmentEntry.ShowAdd = True Then
        Form_Refresh
    End If
...
Public Function Form_Edit()
    ...
    sEnrollmentID = listEntries.CellText(listEntries.Row, 0)
    
    If frmEnrollmentEntry.ShowEdit(sEnrollmentID) = True Then
        Form_Refresh
    End If
...
Public Function Form_Delete()
    ...
    If MsgBox("Are you sure you want to delete this Student entry named   '" & listEntries.CellText(listEntries.Row, 1) & "' ?", vbQuestion + vbYesNo + vbDefaultButton2) <> vbYes Then
        Exit Function
    End If
    ...
    If DeleteEnrollment(sEnrollmentID) = True Then
        Form_Refresh
    Else
        WriteErrorLog Me.Name, "Form_Delete", "Failed on: 'DeleteStudent(lStudentID) = True'"
    End If
